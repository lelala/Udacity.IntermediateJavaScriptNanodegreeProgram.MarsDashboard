        ...
        <header class="">
            <div class="clearfix">
                ...
                <ul class="nav float-md-right justify-content-center">
                    ${["", ...newData.rover].reduce((html, roverObj, index) => html += `
                    <li class="nav-item ">
        ...
        <main role="main"
            ...
            <div class="col-12 col-md-6 col-lg-4">
                <div class="card bg-secondary text-light mb-4 mb-sm-5 shadow-sm">
                    ...
                    <div class="card-body">
                        <p class="card-text">${cameraObj.full_name}</p>
                    </div>
                </div>
            </div>
            `)}
...
onload(() => {
    draw(marsDashboard);
    fetch(`/api/mars_dashboard`)
        .then(res => res.json())
        .then(respObj => {
            Object.assign(marsDashboard, { loading: false }, respObj);
            marsDashboard.rover.forEach((roverObj, index) => {
                roverObj.isActive = index == 0;
            });
            draw(marsDashboard);
        });
});